...
interface CommandPaletteProjectRef {
  readonly environmentId: EnvironmentId;
  readonly projectId: ProjectId;
}

export interface LeadAgentCommandPaletteActionDescriptor {
  readonly kind: "action";
  readonly value: "action:lead-agent";
  readonly searchTerms: ReadonlyArray<string>;
  readonly title: "Open CMD Riker";
  readonly description: string;
  readonly projectRef: CommandPaletteProjectRef;
}

export function resolveCommandPaletteProjectContext<
  TProjectGroup extends { readonly displayName: string },
>(input: {
  readonly routeLeadAgentProjectRef: CommandPaletteProjectRef | null;
  readonly contextualProjectRef: CommandPaletteProjectRef | null;
  readonly projectGroupByTargetKey: ReadonlyMap<string, TProjectGroup>;
  readonly fallbackProjectGroup: TProjectGroup | null;
  readonly projects: ReadonlyArray<Pick<Project, "environmentId" | "id" | "title">>;
  readonly fallbackLeadAgentProject: Pick<Project, "environmentId" | "id" | "title"> | null;
  readonly leadAgentEnvironmentIds: ReadonlyArray<EnvironmentId>;
}): {
  readonly contextualProjectGroup: TProjectGroup | null;
  readonly leadAgentAction: LeadAgentCommandPaletteActionDescriptor | null;
} {
  const paletteProjectRef = input.routeLeadAgentProjectRef ?? input.contextualProjectRef;
  const contextualProjectGroup =
    (paletteProjectRef
      ? input.projectGroupByTargetKey.get(
          `${paletteProjectRef.environmentId}:${paletteProjectRef.projectId}`,
        )
      : null) ?? input.fallbackProjectGroup;
  const leadAgentProject =
    (paletteProjectRef
      ? input.projects.find(
          (project) =>
      ...
      : null) ?? input.fallbackLeadAgentProject;
  const supportsLeadAgent =
    leadAgentProject !== null &&
    input.leadAgentEnvironmentIds.includes(leadAgentProject.environmentId);

  return {
    contextualProjectGroup,
    leadAgentAction: supportsLeadAgent
      ? {
          kind: "action",
          value: "action:lead-agent",
          searchTerms: ["riker", "lead agent", "owner", "session view"],
          title: "Open CMD Riker",
          description: contextualProjectGroup?.displayName ?? leadAgentProject.title,
          projectRef: {
            environmentId: leadAgentProject.environmentId,
            projectId: leadAgentProject.id,
          },
        }
      : null,
  };
