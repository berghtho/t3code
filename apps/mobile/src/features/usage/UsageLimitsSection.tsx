 ...
 * Manual refreshes re-anchor the clock; incoming snapshots can also advance
 * the rendered countdowns and pace. No periodic clock updates are scheduled.
...
export function UsageLimitsSection(props: {
  ...
  // Incoming snapshots also refresh the countdown anchor, without clock ticks.
  const now = Date.now();
  const presentations = useAtomValue(environmentPresentations.presentationsAtom);
