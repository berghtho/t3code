...
describe("applyUsageLimitsUpdate", () => {
  ...
  it("keeps reset credits when a runtime update changes usage", () => {
    const resetCredits = {
      availableCount: 2,
      nextExpiresAt: "2026-09-10T12:00:00.000Z",
    };
    const next = applyUsageLimitsUpdate({
      previous: { ...published, resetCredits },
      checkedAt: "2026-09-03T12:00:05.000Z",
      update: {
        windows: [{ ...session, usedPercent: 55 }],
      },
    });

    expect(next?.windows).toEqual([{ ...session, usedPercent: 55 }, weekly]);
    expect(next?.resetCredits).toEqual(resetCredits);
  });
