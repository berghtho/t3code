...
describe("UsageService", () => {
  ...
  it.live("projects a shared scan to each concurrent client's contract version", () =>
    Effect.gen(function* () {
      const { transcript, settings, home } = yield* setup;
      yield* Effect.promise(() => NodeFSP.writeFile(transcript, claudeLine(1, 5)));
      ...
      const service = yield* UsageService.make.pipe(
        Effect.provide(
          serviceLayers({
            prefix: "usage-service-contract-version-test",
            home,
            settings,
            onRatesFetch: () => {
              ratesFetches += 1;
      ...
      const [current, legacy] = yield* Effect.all(
        [
          service.readSummary({
            ...WINDOW,
      ...
      assert.strictEqual(current.contractVersion, USAGE_CONTRACT_VERSION);
      assert.strictEqual(legacy.contractVersion, USAGE_MERGE_COMPATIBLE_SINCE);
      assert.strictEqual(ratesFetches, 1);
    }).pipe(Effect.scoped),
  );
